...
// 	cabecera cab[10];
	...
	public:
		multi(int numcab,int numest){
			tam1= numcab;
			cab = new cabecera[tam1];
			tam2=numest;
			est = new estudiante[tam2];
		...
		void imprimirNombreAZ(int k);
		void imprimirEdad(int k);
		void imprimirCarrera(string car,int k);
		void imprimirhobby(string hob,int k);
};
...
void multi::imprimirNombreAZ(int k){
	int a = cab[0].pos;
	for(int i=1;i<k;i++){
...
void multi::imprimirCarrera(string car,int k){
	int b=0;
	if(car=="Electronica"){
		b = cab[1].pos;
...
void multi::imprimirhobby(string car,int k){
	int b;
	if(car=="Danza"){
		b=cab[6].pos;
